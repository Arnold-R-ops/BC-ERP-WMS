import { Alert, App as AntdApp, Button, Card, Checkbox, Form, Input, Modal, Select, Space, Table, Tag, Typography } from 'antd';
...
type InvitationType = 'SUPER_ADMIN' | 'ORDINARY_ADMIN';

function PlatformAdminInvitationPanel({ invitationType }: { invitationType: InvitationType }): JSX.Element {
  ...
  const load = useCallback(async () => {
    ...
    setError(false);
    try {
      setInvitations((await listPlatformAdminInvitations())
        .filter((invitation) => invitation.invitationType === invitationType));
    } catch {
      setError(true);
    } finally {
      setLoading(false);
  ...
  const submit = async (values: InvitationFormValues) => {
    ...
    try {
      ...
      const invitation = await createPlatformAdminInvitation({
        ...
        invitationType,
        roleCode: invitationType === 'SUPER_ADMIN'
          ? 'PLATFORM_SUPER_ADMIN'
          : values.jobRoleCode ?? 'PLATFORM_OPERATIONS_ADMIN',
      });
      ...
      form.resetFields();
      setInvitations((await listPlatformAdminInvitations())
        .filter((item) => item.invitationType === invitationType));
    } catch (caught) {
      setError(true);
  ...
  const revoke = async () => {
    ...
    setError(false);
    try {
      await revokePlatformAdminInvitation(revokeTarget.id, revokeReason.trim());
  ...
  const invitationColumns: ColumnsType<PlatformAdminInvitation> = [
    ...
    { title: english ? 'Role' : '岗位', dataIndex: 'roleCode', render: (value: string) => <Tag color={value === 'PLATFORM_SUPER_ADMIN' ? 'red' : 'blue'}>{value}</Tag> },
    ...
    { title: english ? 'Action' : '操作', render: (_, item) => item.status === 'ACTIVE' ? <Button danger disabled={loading} onClick={() => setRevokeTarget(item)}>{english ? 'Revoke' : '撤销'}</Button> : '-' },
  ];

  return <div className="platform-super-invitation-panel">
    <Alert showIcon type={invitationType === 'SUPER_ADMIN' ? 'warning' : 'info'} message={invitationType === 'SUPER_ADMIN'
      ? (english
        ? 'This workflow creates only another independent super administrator; at most two enabled or pending seats are allowed.'
        : '这里只邀请另一名独立超级管理员；启用账号与有效邀请合计最多两个名额。')
      : (english
        ? 'Choose exactly one ordinary job: operations administrator or security auditor. Read/export access is delegated separately after activation.'
        : '普通管理员必须且只能选择一个岗位：平台运营管理员或平台安全审计员。读取／导出权限在激活后另行委派。')} />
    <Alert showIcon type="info" message={english
    ...
    {error ? <Alert action={<Button onClick={() => void load()} size="small">{english ? 'Retry' : '重试'}</Button>} showIcon type="error" message={english ? 'Invitation information or operation could not be completed.' : '邀请信息加载或操作未完成，请重试。'} /> : null}
    <Card title={invitationType === 'SUPER_ADMIN'
      ? (english ? 'Invite another super administrator' : '邀请第二名超级管理员')
      : (english ? 'Invite an ordinary administrator' : '邀请普通管理员')}>
      <Form form={form} layout="vertical" onFinish={(values) => void submit(values)}>
        ...
        {invitationType === 'ORDINARY_ADMIN' ? <Form.Item
          name="jobRoleCode"
          label={english ? 'Job role' : '平台岗位'}
          rules={[{ required: true }]}
        >
          <Select options={[
    ...
    <Card loading={loading} title={english ? 'Invitation history' : '邀请记录'}>
      <Table columns={invitationColumns} dataSource={invitations} pagination={false} rowKey="id" scroll={{ x: 860 }} size="small" />
    </Card>
    <Modal
      closable={false}
    ...
    <Modal
      okButtonProps={{ danger: true, disabled: !revokeReason.trim(), loading }}
      okText={english ? 'Revoke invitation' : '确认撤销邀请'}
      onCancel={() => { setRevokeTarget(null); setRevokeReason(''); }}
      onOk={() => void revoke()}
      open={Boolean(revokeTarget)}
      title={english ? 'Revoke invitation' : '撤销邀请'}
    >
      <Typography.Paragraph type="secondary">
        {revokeTarget ? `${revokeTarget.displayName} · ${revokeTarget.email}` : ''}
      </Typography.Paragraph>
      <Input.TextArea
        maxLength={500}
        onChange={(event) => setRevokeReason(event.target.value)}
        placeholder={english ? 'Required revocation reason' : '必填：撤销原因'}
        rows={3}
        showCount
        value={revokeReason}
      />
...
export function PlatformOrdinaryAdminInvitationPanel(): JSX.Element {
  return <PlatformAdminInvitationPanel invitationType="ORDINARY_ADMIN" />;
}

export function PlatformSuperAdminInvitationPanel(): JSX.Element {
  return <PlatformAdminInvitationPanel invitationType="SUPER_ADMIN" />;
}
