...
public interface ProductSkuRepository extends JpaRepository<ProductSku, Long> {
     ...
     * @return Optional<ProductSku> 商品对象（可能为空）
     */
     ...
     * @return 命中的商品
     */
    @Query("SELECT p FROM ProductSku p WHERE REPLACE(UPPER(p.barcode), ' ', '') = :normalizedBarcode")
    Optional<ProductSku> findByNormalizedBarcode(@Param("normalizedBarcode") String normalizedBarcode);

    /**
     * SKU 映射人工确认的候选推荐（P1 批次2）：条码前缀或名称关键词匹配
     */
    List<ProductSku> findTop10ByBarcodeStartingWithIgnoreCaseOrSkuNameContainingIgnoreCase(String barcodePrefix, String nameKeyword);
     ...
     */
    List<ProductSku> findByNameContaining(String keyword);
     ...
     * @return 该分类下的所有商品
     */
    List<ProductSku> findByProductCategoryId(Long categoryId);
     ...
     * @return 该供应商的所有商品
     */
    List<ProductSku> findBySupplier(String supplier);
     ...
     *
     * JPQL 语法说明：
     * 1. SELECT p: 查询 ProductSku 实体
     * 2. FROM ProductSku p: 从 ProductSku 表查询（p 是别名）
    ...
    @Query("SELECT p FROM ProductSku p " +
           "WHERE p.id IN (" +
           "  SELECT b.productSku.id FROM InventoryBatch b " +
           "  WHERE b.active = true AND b.location IS NOT NULL " +
           "  GROUP BY b.productSku.id " +
           "  HAVING SUM(b.quantity) < " +
           "    (SELECT p2.minStock FROM ProductSku p2 WHERE p2.id = b.productSku.id)" +
           ")")
    List<ProductSku> findLowStockProducts();
    ...
    @Query("SELECT p FROM ProductSku p " +
           ...
           "    (SELECT p2.minStock FROM ProductSku p2 WHERE p2.id = b.productSku.id)" +
           ")")
    List<ProductSku> findLowStockProductsBySupplier(@Param("supplier") String supplier);
     ...
     */
    @Query("SELECT p FROM ProductSku p ORDER BY p.leadTime DESC")
    List<ProductSku> findAllOrderByLeadTimeDesc();
     ...
     */
    @Query("SELECT p FROM ProductSku p WHERE p.minStock BETWEEN :minThreshold AND :maxThreshold")
    List<ProductSku> findByMinStockBetween(@Param("minThreshold") Integer minThreshold,
                                         @Param("maxThreshold") Integer maxThreshold);

    boolean existsBySkuCode(String skuCode);

    @Query(value = "SELECT nextval('product_sku_code_seq')", nativeQuery = true)
    Long nextSkuCodeSequence();
}
