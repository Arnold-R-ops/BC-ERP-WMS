...
/**
 * Purchase Order Entity (采购单主表实体)
 *
 * Master-Detail 架构的主表，包含采购单头信息。
 *
 * Business Flow (三阶段状态流转):
 * 1. ORDERING: 创建采购单，上传 Excel 或手动录入
 * 2. IN_TRANSIT: 确认并生成批次码（Hashids）
 * 3. PARTIALLY_RECEIVED / COMPLETED: 实物入库（分批入库支持）
 *
 * Features:
 * - 支持分批入库（receivedQuantity 累加）
 * - 支持状态回退（IN_TRANSIT → ORDERING，批次码作废）
 * - 审计日志记录（auditLog）
 * - 隐私字段掩码（supplier, totalCost 对 STAFF 角色隐藏）
 *
 * @author WMS Team
 * @since 2025-01-13
...
public class PurchaseOrder extends BaseEntity {
    ...
    /**
     * 总采购数量（汇总所有明细项的 orderedQuantity）
     *
     ...
     * 计算总成本
     *
     * 汇总所有明细项的 (orderedQuantity × unitCost)
     *
     * @return 总成本
     */
     ...
     * 检查是否所有明细都已完全入库
     *
     * 判断规则: 所有明细项的 receivedQuantity == orderedQuantity
     *
     * @return true 如果所有明细都已完全入库
     */
