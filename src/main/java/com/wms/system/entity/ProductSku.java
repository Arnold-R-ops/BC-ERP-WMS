...
/**
 * ProductSku SKU Entity (Stock Keeping Unit - 库存保有单元)
 *
 * V3.3 Architecture: SKU Level in SPU-SKU Hierarchy
 *
 * 概念说明：
 * - SKU (Stock Keeping Unit): 库存保有单元，代表具体可销售的商品规格
 * - 每个 SKU 必须归属于一个 SPU (Standard ProductSku Unit - 产品家族)
...
@Entity
@SequenceGenerator(
    name = "product_sku_code_sequence",
    sequenceName = "product_sku_code_seq",
    allocationSize = 1
)
@SQLDelete(sql = "UPDATE product_skus SET is_deleted = true, version = version + 1 WHERE id = ? AND version = ?")
@SQLRestriction("is_deleted = false")
@Table(
    name = "product_skus",
    indexes = {
        @Index(name = "idx_product_skus_company_barcode", columnList = "company_id,barcode"),
        @Index(name = "idx_product_skus_company_name", columnList = "company_id,name"),
        @Index(name = "idx_product_skus_company_product", columnList = "company_id,product_id")
    },
    uniqueConstraints = {
        @UniqueConstraint(name = "uk_product_skus_company_code", columnNames = {"company_id", "sku_code"}),
        @UniqueConstraint(name = "uk_product_skus_company_barcode", columnNames = {"company_id", "barcode"})
    }
)
public class ProductSku extends BaseEntity {
    ...
    /** Stable internal SKU identifier. It is generated once and never edited. */
    @NotBlank(message = "SKU code is required")
    @Size(max = 50)
    @Column(name = "sku_code", nullable = false, length = 50, updatable = false)
    private String skuCode;

    // ========== V3.3 SPU-SKU 层级关联 (SPU-SKU Hierarchy) ==========

    /**
     * 关联的 SPU (Standard ProductSku Unit - 产品家族)
     *
     * V3.3 架构说明：
     * - 每个 SKU 必须归属于一个 SPU
