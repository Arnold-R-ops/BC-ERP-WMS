...
1. 历史凭证扫描与本机数据库轮换已完成；旧数据库密码失效，应用切换为 `wms_app`，当前发布凭证门禁为 `PASS`。轮换工具和证据已推送，远程 CI `31412241192` 全部通过。
2. 单独规划 Flyway 对 PostgreSQL 18 的支持版本升级。
3. 用户明确批准后才可创建发布标签或部署。

下一任务仅为用户单独批准的 P2 标签或部署决策。按 `CODEX_TASK_MODEL_POLICY` 建议使用 **GPT-5.6 Sol + XHigh（L5）**。
