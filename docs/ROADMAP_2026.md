...
Java 17 + Spring Boot 3.2.11 + PostgreSQL 核心服务，配套 React + Ant Design 管理端 V1.1。采购→批次库存（FEFO）→销售风控→预留/Backorder→出库→盘点闭环完整；商品、类别、Client/Consumer、供应商、Shopify 只读渠道链路与 IAM 管理端已接入。P0/P0.5/P1 已关闭；用户已确认的 P2 功能范围完成收尾验收，普通业务模板权限基线已按暂定方案落地，Localization 继续暂缓。
...
> **2026-08-09 最终只读收尾审查：当前确认范围通过。** 上述契约漂移和执行器问题已关闭；权限申请、采购单 `ORDERING` 编辑、历史测试数据可审计归档及归档后销售事实刷新均已落地。开发库迁移到 `V4.48`，后端 125 个套件/1287 项、前端 44 个文件/143 项全部通过，实时 OpenAPI、快照和 TypeScript 类型一致。员工/采购真实页面留证属于非阻塞复核；普通业务模板权限基线当时延期，后于 2026-08-10 按暂定方案完成。工作区尚未提交，且本轮未获 Git 发布授权，因此“功能范围通过”不等于已推送或已创建 P2 标签。详见 `docs/P2_FINAL_CLOSURE_AUDIT_2026-08-09.md`。
> **2026-08-10 普通业务模板暂定基线完成。** `V4.49` 已将 `GENERAL_MANAGER`、`SALESPERSON` 分别收敛为 40 项和 15 项精确直接权限并开放导入；仓库角色和受保护特权角色不变。当前先以该方案完成全部验收，之后在后续版本另开专项修改普通业务模板与受保护特权角色。验收证据见 `docs/frontend/ORDINARY_ROLE_BASELINE_ACCEPTANCE_2026-08-10.md`。
> **2026-08-10 全部确认范围最终验收通过。** 当前源码独立编译、Java 1289 项、前端 142 项、生产构建、实时 OpenAPI/TypeScript 契约和 HTTP 只读预检均通过。功能范围已关闭，正式发布仍需标准 Maven/CI 打包、工作区变更分组、敏感信息复核及 Git 授权。详见 `docs/P2_FINAL_ACCEPTANCE_AUDIT_2026-08-10.md`。
> **2026-08-10 发布准备完成。** 244 个候选入口已全部归类；标准 PostgreSQL/Maven 与 pnpm 前端 CI、冻结锁、类型漂移门禁和 release gate 已建立。尚未执行 Git 暂存、提交、推送、标签或远程 CI，后续必须先取得用户明确授权。详见 `docs/P2_RELEASE_CHANGESET_PLAN_2026-08-10.md`。
> **2026-08-10 发布分支门禁通过。** 经用户授权，244 个候选入口已按三组提交并推送到 `origin/2-1G`；首次 GitHub Actions 运行 `31393616581` 的后端 Maven、前端验证和 Release gate 全部通过。尚未创建 P2 标签或部署，正式发布仍需历史凭证轮换确认和用户单独批准。
> **2026-08-10 历史凭证扫描完成但轮换门禁阻塞。** 全部 38 个可达提交未发现需复核的高置信私钥或平台令牌；本机 JWT 密钥已不匹配历史值，但数据库密码仍匹配 Git 历史旧值，且数据库账号未显式切换到 `wms_app`。在受控窗口完成数据库端轮换前不得创建 P2 标签或部署。
> **2026-08-11 数据库凭证门禁通过。** 旧 `postgres` 密码已轮换并验证失效；`wms_app` 接管两套本机数据库和 public 对象，四项高权限标志均为 `false`。后端以新凭证重启，V4.49、健康、OpenAPI、回滚型 DDL 探针和脱敏历史门禁均通过；恢复凭证仅保存在 Git 忽略的当前用户 DPAPI 包中。
> **2026-08-11 轮换证据远程门禁通过。** 安全审计与轮换提交已推送，GitHub Actions `31412241192` 的 Backend verify、Frontend verify 和 Release gate 全部成功。P2 技术发布门禁已关闭，标签或部署仍需用户单独批准。
...
- **采购单创建后编辑（2026-08-09 已完成）**：已创建且仍处于 `ORDERING` 的采购单可再次编辑供应商、预计到货日、备注和采购明细；保存使用版本号进行并发校验并追加审计。编辑不确认 ASN、不收货、不修改库存；已有历史批次的明细禁止换商品、改数量或删除。
